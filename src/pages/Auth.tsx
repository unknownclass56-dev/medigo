...
const AuthPage = () => {
  ...
  const handleGoogleSignIn = async () => {
    if (isPreviewOrigin) {
      ...
      targetUrl.searchParams.set("google_oauth", "1");
      const popup = window.open(targetUrl.toString(), "_blank", "noopener,noreferrer");
      if (!popup) {
        toast.error("Popup block ho gaya. Published tab me auth page kholo aur fir Google sign-in karo.");
      }
      return;
    }
  ...
  useEffect(() => {
    if (params.get("google_oauth") !== "1" || googleAutoStarted.current) return;
    googleAutoStarted.current = true;
    void startGoogleSignIn(PUBLISHED_APP_URL);
  }, [params]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-secondary/40 via-background to-background">
      <div className="container max-w-md py-10">
        ...
        <Card className="shadow-elegant">
          <CardHeader>
            <CardTitle className="text-2xl">Welcome to MediHealth</CardTitle>
          ...
          <CardContent>
            ...
            <Button
              variant="outline"
              className="h-11 w-full items-center justify-center gap-3 border-2 font-bold transition-all"
